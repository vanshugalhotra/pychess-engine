...
class Board:
    ...
    def get_fen(self) -> str:
        ...
        fen += " "
        fen += "w" if self.side == Colors.WHITE else "b"
        fen += " "

        castling = ""
        if self.castlePerm & Castling.WKCA:
            castling += "K"
        if self.castlePerm & Castling.WQCA:
            castling += "Q"
        if self.castlePerm & Castling.BKCA:
            castling += "k"
        if self.castlePerm & Castling.BQCA:
            castling += "q"
        ...
        if self.enPas == Squares.NO_SQ:
            ...
            file = FilesBrd[self.enPas]
            rank = RanksBrd[self.enPas]
            fen += f"{chr(ord('a') + file)}{rank + 1}"

        fen += f" {self.fiftyMove} {self.hisPly // 2 + 1}"
